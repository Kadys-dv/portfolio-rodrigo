...
function initProjectSequence() {
  const projects = document.querySelectorAll('.case-study, .api-case, .helpdesk-case, .ritmorax-case');
  projects.forEach((project) => {
    const copy = project.querySelector('.case-copy, .api-copy, .helpdesk-copy, .ritmorax-copy');
    if (!copy) return;
    [...copy.querySelectorAll('h3, h4, .project-metrics, .project-links')].forEach((element, index) => {
