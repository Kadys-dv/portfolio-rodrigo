...
for (const viewport of [{ name: 'desktop', width: 1440, height: 1000 }, { name: 'mobile', width: 412, height: 915 }]) {
  ...
  await page.screenshot({ path: `${process.env.TEMP}\\portfolio-about-${viewport.name}.png`, fullPage: false });
  console.log(JSON.stringify({ viewport: viewport.name, layout, errors }));
  if (Math.abs(layout.portraitCenter - layout.headingCenter) > 2 || errors.length) process.exitCode = 1;
  await page.close();
}
